...
(function (factory) {
	...
	$.fn.jR3DCarousel = function(options){
		...
		(function setup(){
			...
			function _createControls(){
				...
				_previousButton.add(_nextButton).appendTo(_container)
							   .css({position: 'absolute', top:'42%', zIndex:1, display: 'inline-block', padding: '1.2em', boxShadow: '2px 2px 0 rgba(255,85,0,0.9) inset', cursor:'pointer'})
							   .hide();
			...
			function _createNavigation(){
				...
				_navigation.find('.nav').css({ display: 'inline-block', margin: '5px', cursor: 'pointer', backgroundColor: 'rgba(255, 85, 0, 0.77)', width: '12px', height: '12px', transition: 'all '+_settings.animationDuration+'ms ease' })
		...
		function _fade(targetSlideIndex){
			...
			/* set active nav icon */
			_container.find('.nav').css({  backgroundColor: 'rgba(255, 85, 0, 0.77)' })
